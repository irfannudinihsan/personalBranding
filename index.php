<?php get_header(); ?>

<!-- section hero -->
<section id="home" class="pt-40 bg-blue-50  bg-opacity-70 relative  dark:bg-slate-900 font-poppins">
	<div class="container mx-auto  ">
		<div class="flex flex-wrap center">
			<!-- description hero -->
			<div class="w-full px-2 lg:w-2/3 flex flex-col justify-center lg:pr-10 xl:pr-40 2xl:pr-60 ">
				<p class="font-semibold text-lg text-blue-900 poppins inline-block dark:text-slate-400">Hi, Im</p>
				<h1 class="font-semibold text-4xl lg:text-5xl text-blue-400 mb-3 font-poppins">Irfannudin Ihsan</h1>

				<p class="font-semibold text-lg text-blue-900 mb-3 font-poppins px-2 p-1 w-fit rounded-md bg-blue-100 inline-block ">Web Developer</p>


				<p class="text-slate-500 font-poppins text-sm font-normal lg:me-32 ">You call me Irfan. I am interested in Web Developer and UI/UX Designer. I usually create designs using Figma and then implement them in HTML, CSS, Javascript, React JS, or Wordpress. Thank you for your support and if you would like to collaborate.
				</p>

				<div class="mt-6 relative ">
					<a href="https://drive.google.com/file/d/1zALQ3AXHAnOkZSBaCBSaQmnZPd0AOZWo/view?usp=sharing" target="blank" class="inline-block bg-blue-400 hover:bg-blue-600 duration-300 py-3 px-8  rounded-lg text-white mb-3 font-semibold text-sm font-poppins ">View CV</a>
					<!-- <a href="https://drive.google.com/drive/folders/1GNn4Ubk0v8w9SyLQ_kYNxn75YxbbPISl?usp=drive_link" class="inline-block bg-transparant py-3 px-8 mx-1 rounded-lg text-blue-500  border-2 border-blue-500 text-sm font-semibold font-poppins" target="_blank">Contact</a> -->
				</div>


			</div>

			<!-- image hero -->
			<div class="w-full lg:mt-20 xl:mt-0 lg:w-1/3">
				<div class="relative w-full bottom-0">

					<img src="<?php bloginfo('template_url') ?>/images/photo.png" alt="irfannudin ihsan" class="xs:w-4/6 sm:w-4/6 md:w-4/6 lg:w-full xs:left-1/2 xs:transform xs:-translate-x-1/2 sm:left-1/2 sm:transform sm:-translate-x-1/2 md:left-1/2 md:transform md:-translate-x-1/2 z-10 absolute bottom-0 ">

					<span class=" bottom-0 w-[240px] xs:w-[360px] z-0 w-xl left-1/2 -translate-x-1/2">
						<img class="w-5/6 mx-auto mt-20 xs:w-4/6 lg:w-5/6" src="<?php bloginfo('template_url') ?>/images/blob_blob.svg">
					</span>
				</div>

			</div>


		</div>
</section>





<!-- sosial media -->


<section id="sosial media ">

	<div class="container font-poppins  ">

		<div class="mx-auto dark:bg-slate-800 w-full md:flex flew-wrap lg:w-[1200px]   bg-white    p-4 lg:p-5 rounded-none lg:rounded-lg lg:-mt-6 xl:-mt-9 - z-10 absolute left-1/2 transform -translate-x-1/2 shadow-lg shadow-slate-50 dark:shadow-slate-900   ">

			<div class="flex w-full justify-center my-4 snap-center">
				<a href="https://github.com/irfannudinihsan" target="_blank">
					<div class="p-3 bg-blue-400 rounded-full mx-1 hover:bg-blue-600">


						<img width="28" height="28" src="<?php bloginfo('template_url') ?>/images/github.svg" alt="" class="fill-blue-400">

					</div>
				</a>

				<div class="mx-1">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white ">Github </h1>
					<p class="text-sm font-normal text-slate-500 dark:text-blue-100 "><a href="https://github.com/irfannudinihsan" target="_blank">irfannudinihsan</a></p>


				</div>

			</div>
			<div class="flex w-full justify-center my-4 snap-center">
				<a href="https://dribbble.com/irfannudinihsan" target="_blank">
					<div class="p-3 bg-blue-400 rounded-full mx-1 hover:bg-blue-600">

						<img width="28" height="28" src="<?php bloginfo('template_url') ?>/images/dribble.svg" alt="">


					</div>
				</a>

				<div class="mx-1">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white">Dribbble </h1>
					<p class="text-sm font-normal text-slate-500 dark:text-blue-100"><a href="https://dribbble.com/irfannudinihsan" target="_blank">irfannudinihsan</a></p>


				</div>

			</div>
			<div class="flex w-full justify-center my-4 snap-center">
				<a href="https://www.linkedin.com/in/irfannudin-ihsan" target="_blank">
					<div class="p-3 bg-blue-400 rounded-full mx-1 hover:bg-blue-600">

						<img width="28" height="28" src="<?php bloginfo('template_url') ?>/images/linkedin.svg" alt="">




					</div>
				</a>

				<div class="mx-1">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white">Linkedin </h1>
					<p class="text-sm font-normal text-slate-500 dark:text-blue-100 "> <a href="https://www.linkedin.com/in/irfannudin-ihsan" target="_blank">Irfannudin Ihsan</a> </p>

				</div>

			</div>


			<div class="flex w-full justify-center my-4 snap-center">

				<a href="https://www.instagram.com/irfannudinihsan/" target="_blank">
					<div class="p-3 bg-blue-400 rounded-full mx-1 hover:bg-blue-600">

						<img width="28" height="28" src="<?php bloginfo('template_url') ?>/images/instagram.svg" alt="ig">



					</div>

				</a>
				<div class="mx-1">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white">Instagram </h1>
					<p class="text-sm font-normal text-slate-500 dark:text-blue-100"><a href="https://www.instagram.com/irfannudinihsan/" target="_blank">irfannudinihsan</a></p>




				</div>

			</div>

		</div>


	</div>


</section>

<!-- section service -->
<section id="service" class="pt-96 pb-20 sm:pt-60  sm:pb-20 dark:bg-slate-950 ">

	<div class="container font-poppins mx-auto pt-0 sm:pt-40 md:pt-0   ">

		<div class="grid grid-cols-1 xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 xl:gap-4 gap-3">

			<!-- item grid -->
			<div class="mb-4">
				<h2 class="font-semibold text-3xl md:text-3xl lg:text-3xl text-blue-400 mb-4 font-poppins">Services for collaboration</h2>
				<p class="text-slate-500 font-poppins text-sm font-normal ">
					You can collaborate with me, here are the services I offer. Don't hesitate to contact me.
				</p>

			</div>


			<!-- item grid -->
			<div class="dark:bg-slate-800 transition duration-700 ease-in-out dark:hover:bg-slate-900 hover:bg-blue-50  flex flex-col items-center  rounded-xl shadow-lg p-10  shadow-slate-100 dark:shadow-slate-900">
				<div class="p-3 bg-blue-100 rounded-full  inline-block mb-3">
					<img width="40" height="40" src="<?php bloginfo('template_url') ?>/images/webdev.svg" alt="">
				</div>

				<div class="mb-3 px-2 ">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white text-center">Web Developer</h1>
				</div>



				<div class="mb-3">

					<p class="text-slate-400 text-sm text-center">Create dynamic websites, can be Frontend, Backend or FullStack</p>

				</div>

				<div class=" w-full flex justify-center flex-wrap">



					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/html.svg" alt="">
					<img class="mx-2 my-2 " width="28" height="28" src="<?php bloginfo('template_url') ?>/images/css.svg" alt="">
					<img class="mx-2 my-2 " width="28" height="28" src="<?php bloginfo('template_url') ?>/images/js.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/bt.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/tw.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/rj.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/lrv.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/wp.svg" alt="">

				</div>

			</div>
			<div class="dark:bg-slate-800 transition duration-700 ease-in-out dark:hover:bg-slate-900 hover:bg-blue-50  flex flex-col items-center  rounded-xl shadow-lg p-10  shadow-slate-100 dark:shadow-slate-900">
				<div class="p-3 bg-blue-100 rounded-full  inline-block mb-3">
					<img width="40" height="40" src="<?php bloginfo('template_url') ?>/images/webdes.svg" alt="">
				</div>

				<div class="mb-3 px-2 ">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white text-center ">Web Designer</h1>
				</div>

				<div class="mb-3">

					<p class="text-slate-400 text-sm text-center">Create static website that prioritizes website User Inteface Website such as the company profile</p>

				</div>

				<div class=" w-full flex justify-center flex-wrap">





					<img class=" mx-2 my-2 " width="28" height="28" src="<?php bloginfo('template_url') ?>/images/html.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/css.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/bt.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/tw.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/wp.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/wf.svg" alt="">

				</div>

			</div>
			<div class="dark:bg-slate-800 transition duration-700 ease-in-out dark:hover:bg-slate-900 hover:bg-blue-50  flex flex-col items-center  rounded-xl shadow-lg p-10  shadow-slate-100 dark:shadow-slate-900 ">
				<div class="p-3 bg-blue-100 rounded-full  inline-block mb-3">
					<img width="40" height="40" src="<?php bloginfo('template_url') ?>/images/uiux.svg" alt="">
				</div>

				<div class="mb-3 px-2 ">
					<h1 class="text-lg font-semibold text-slate-700 dark:text-white text-center">UI/UX Designer</h1>
				</div>

				<div class="mb-3">

					<p class="text-slate-400 text-sm text-center">Create User Interface, assets, illustration content and prototype</p>

				</div>

				<div class=" w-full flex justify-center flex-wrap">



					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/fg.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/cv.svg" alt="">
					<img class="mx-2 my-2" width="28" height="28" src="<?php bloginfo('template_url') ?>/images/ai.svg" alt="">


				</div>


			</div>




		</div>


	</div>

</section>




<section id="article" class="font-poppins pt-20 pb-20 dark:bg-slate-950">

	<div class="container mx-auto ">
		<!-- title section -->
		<div class="mb-4 flex-none md:flex justify-between ">

			<div>

				<h2 class="font-semibold text-3xl md:text-3xl lg:text-3xl text-blue-400 mb-4 font-poppins">Work</h2>
				<p class="text-slate-500 font-poppins text-sm font-normal ">
					Check the projects i've worked on
				</p>

			</div>

			<div class="mt-6 ">
				<a href="http://latihan1.local/work/" class="inline-block bg-blue-400 hover:bg-blue-600 duration-300 py-3 px-8  rounded-lg text-white mb-3 font-semibold text-sm font-poppins ">Work More</a>

			</div>

		</div>



		<div class=" grid grid-cols-1 xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-3 gap-3 ">

			<!-- test index copy -->
			<?php if (have_posts()) : ?>
				<?php
				while (have_posts()) :
					the_post();
				?>

					<?php get_template_part('template-parts/content', get_post_format()); ?>

				<?php endwhile; ?>

			<?php endif; ?>

		</div>





</section>







<!-- section Contact -->

<section id="contact" class="pt-20 pb-40 font-poppins bg-blue-50  bg-opacity-90  dark:bg-slate-900">

	<div class="container mx-auto ">

		<!-- title section -->
		<div class="flex flex-wrap center">

			<!-- description section -->
			<div class="w-full lg:w-1/3 mb-16  ">
				<h2 class="font-semibold text-3xl md:text-3xl lg:text-3xl text-blue-400 mb-4 font-poppins">

					My Contact
				</h2>


				<p class="text-slate-500 font-poppins text-sm font-normal ">
					If you need Web Developer or UI/UX Designer, contact me
				</p>
			</div>


			<!-- content contact -->
			<div class="w-full lg:w-2/3 ">

				<!-- grid grid-cols-1 xs:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 xl:gap-4 gap-3 -->


				<div class="grid grid-cols-1 xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-x-8 gap-y-16 ">

					<!-- item grid -->
					<div class="flex flex-col items-center justify-center rounded-xl  aspect-video bg-white  relative dark:bg-slate-800  ">

						<div class="absolute -top-8">
							<a href="mailto:irfannudinihsan56@gmail.com" target="_blank">
								<div class="p-6 bg-blue-200 hover:bg-blue-600 rounded-full mx-auto inline-block mb-2">
									<img class="w-10 xs:w-16 xl:w-20" src="<?php bloginfo('template_url') ?>/images/email-blue.svg" alt="">
								</div>

							</a>
						</div>

						<div class="text-center mt-10 ">

							<h2 class="text-lg font-semibold text-slate-700 lg:mt-4 lg:text-lg xl:text-xl dark:text-white ">Email</h2>
							<a href="mailto:irfannudinihsan56@gmail.com" target="_blank">
								<p class="text-slate-400 text-sm text-center lg:text-md xl:text-lg">irfannudinihsan56@gmail.com</p>
							</a>

						</div>

					</div>
					<!-- end item grid -->



					<!-- wa new -->
					<!-- item grid -->
					<div class="flex flex-col items-center justify-center rounded-xl  aspect-video bg-white  relative dark:bg-slate-800  ">

						<div class="absolute -top-8">
							<a href="https://wa.me/6285831008476" target="_blank">
								<div class="p-6 bg-blue-200 hover:bg-blue-600 rounded-full mx-auto inline-block mb-2">
									<img class="w-10 xs:w-16 xl:w-20" src="<?php bloginfo('template_url') ?>/images/wa-blue.svg" alt="">
								</div>

							</a>
						</div>

						<div class="text-center mt-10 ">

							<h2 class="text-lg font-semibold text-slate-700 lg:mt-4 lg:text-lg xl:text-xl dark:text-white ">Whatsapp</h2>
							<a href="https://wa.me/6285831008476" target="_blank">
								<p class="text-slate-400 text-sm text-center lg:text-md xl:text-lg">+628-5831-0084-76</p>
							</a>

						</div>

					</div>
					<!-- end item grid -->


					<!-- linkedin new -->
					<!-- item grid -->
					<div class="flex flex-col items-center justify-center rounded-xl  aspect-video bg-white  relative dark:bg-slate-800  ">

						<div class="absolute -top-8">
							<a href="https://www.linkedin.com/in/irfannudin-ihsan" target="_blank">
								<div class="p-6 bg-blue-200 hover:bg-blue-600 rounded-full mx-auto inline-block mb-2">
									<img class="w-10 xs:w-16 xl:w-20" src="<?php bloginfo('template_url') ?>/images/linkedin-blue.svg" alt="">
								</div>

							</a>
						</div>

						<div class="text-center mt-10 ">

							<h2 class="text-lg font-semibold text-slate-700 lg:mt-4 lg:text-lg xl:text-xl dark:text-white ">LinkedIn</h2>
							<a href="https://www.linkedin.com/in/irfannudin-ihsan" target="_blank">
								<p class="text-slate-400 text-sm text-center lg:text-md xl:text-lg">irfanudinihsan</p>
							</a>

						</div>

					</div>
					<!-- end item grid -->

					<!-- ig new -->
					<!-- item grid -->
					<div class="flex flex-col items-center justify-center rounded-xl  aspect-video bg-white  relative dark:bg-slate-800  ">

						<div class="absolute -top-8">
							<a href="https://www.instagram.com/irfannudinihsan/" target="_blank">
								<div class="p-6 bg-blue-200 hover:bg-blue-600 rounded-full mx-auto inline-block mb-2">
									<img class="w-10 xs:w-16 xl:w-20" src="<?php bloginfo('template_url') ?>/images/ig-blue.svg" alt="">
								</div>

							</a>
						</div>

						<div class="text-center mt-10 ">

							<h2 class="text-lg font-semibold text-slate-700 lg:mt-4 lg:text-lg xl:text-xl dark:text-white ">Instagram</h2>
							<a href="https://www.instagram.com/irfannudinihsan/" target="_blank">
								<p class="text-slate-400 text-sm text-center lg:text-md xl:text-lg">irfannudinihsan</p>
							</a>

						</div>

					</div>
					<!-- end item grid -->








				</div>



			</div>




		</div>

	</div>

</section>






<section id="" class="pt-40 dark:bg-slate-950">

	<div class="container mx-auto ">

		<div class="flex flex-wrap center justify-between">


			<!-- card customer -->
			<div class="w-full lg:w-1/2    ">

				<!-- for email -->

				<div class="relative mt-10 -z-1 ">
					<img src="<?php bloginfo('template_url') ?>/images/email.png" alt="irfannudin ihsan" class="max-w-full mx-auto pr-20 pl-2 pb-10  ">
				</div>



			</div>

			<!-- card customer -->
			<div class="w-full lg:w-1/2 mx-auto  ">



				<?php echo do_shortcode('[wpforms id="4094"]'); ?>

				<!-- 			<form action="">

					<div class="mb-4">

						<label for="" form="email" class="block font-poppins font-normal mb-1  dark:text-slate-100">Your Name</label>
						<input type="email" id="email" class="border bg-gray-50 border-gray-300 w-full px-2 py-3 block text-gray-900 text-sm  rounded-lg placeholder:text-slate-500 " />



					</div>
					<div class="mb-4">

						<label for="" form="email" class="block font-poppins font-normal mb-1  dark:text-slate-100">Your Email</label>
						<input type="email" id="email" class="border bg-gray-50 border-gray-300 w-full px-2 py-3 block text-gray-900 text-sm  rounded-lg placeholder:text-slate-500 " />



					</div>
					<div class="mb-4">

						<label for="" form="email" class="block font-poppins font-normal mb-1  dark:text-slate-100">Your Subject</label>
						<input type="email" id="email" class="border bg-gray-50 border-gray-300 w-full px-2 py-3 block text-gray-900 text-sm  rounded-lg placeholder:text-slate-500 " />



					</div>
					<div class="mb-4">

						<label for="" form="email" class="block font-poppins font-normal mb-1  dark:text-slate-100">Your Message</label>

						<textarea type="email" id="email" name="" id="" cols="20" rows="6" class="border bg-gray-50 border-gray-300 w-full px-2 py-10 block text-gray-900 text-sm  rounded-lg placeholder:text-slate-500 ">


		</textarea>

					</div>






					<div class="mb-4">
						<a href="" class="inline-block bg-blue-500 py-3 px-8 rounded-full text-white mb-3 font-semibold text-sm font-poppins">Submit</a>
					</div>

				</form> -->


			</div>








		</div>

	</div>

</section>




<?php
get_footer();
